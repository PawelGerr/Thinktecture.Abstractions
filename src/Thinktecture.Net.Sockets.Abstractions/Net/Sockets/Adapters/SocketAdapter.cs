...
namespace Thinktecture.Net.Sockets.Adapters
{
   ...
   public class SocketAdapter : AbstractionAdapter<Socket>, ISocket
   {
      ...
      /// </remarks>
      public SocketAdapter(SafeSocketHandle handle)
         : this(new Socket(handle))
      {
      }
#endif
      ...
      {
         Implementation.Connect(addresses.ToNotNullImplementation<IIPAddress, IPAddress>(), port);
      }
      ...
      {
         Implementation.SetRawSocketOption(optionLevel, optionName, optionValue);
      }

      /// <inheritdoc />
      public int GetRawSocketOption(int optionLevel, int optionName, Span<byte> optionValue)
      {
         return Implementation.GetRawSocketOption(optionLevel, optionName, optionValue);
      }
#endif

#if NETCOREAPP || NET5_0
      /// <inheritdoc />
      public SafeSocketHandle SafeHandle => Implementation.SafeHandle;
#endif
   }
}
